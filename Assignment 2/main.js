...
function initTextures() {
    ...
    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"textures/space.png") ;

    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"textures/sun.png") ;
    
    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"textures/earth.png") ;

    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"textures/moon.png") ;

    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"textures/space_blink.png") ;

    textureArray.push({}) ;
    loadFileTexture(textureArray[textureArray.length-1],"textures/astreoid.png") ;

    for (let i = 0; i < 9; i++) {
        textureArray.push({}) ;
        loadFileTexture(textureArray[textureArray.length-1],"textures/tile00"+i+".png") ;
    }
...
// printes frame rate / 2 and resets frame count
function printFrameRate()
{
    console.log("Framerate: "+Math.round(fps/2)) // divide fps by 2
    fps = 0 // reset counter
}

// starts printFrameRate to run every 2 seconds
var frame_rate = setInterval(printFrameRate, 2000)
...
function spawnSpace()
{
    gPush()
    {
        ...
        // load space/stars texture
        gl.activeTexture(gl.TEXTURE3)
        gl.bindTexture(gl.TEXTURE_2D, textureArray[3].textureWebGL)
        gl.uniform1i(gl.getUniformLocation(program, "albedoMap"), 3)
        gl.activeTexture(gl.TEXTURE7)
        gl.bindTexture(gl.TEXTURE_2D, textureArray[7].textureWebGL)
        gl.uniform1i(gl.getUniformLocation(program, "starMap"), 7)
...
function spawnSun()
{
    gPush()
    {
        ...
        gl.uniform1i(gl.getUniformLocation(program, "albedoMap"), 4)
...
function spawnEarth()
{
    gPush()
    {
        ...
        gl.uniform1i(gl.getUniformLocation(program, "albedoMap"), 5)
        ...
        gPush()
        {
            ...
            // second component moon
            gPush()
            {
                gRotate(-TIME*200, 0, 0,1) ;

                setColor(vec4(.75, .75, .75, 1))

                gl.activeTexture(gl.TEXTURE6)
                gl.bindTexture(gl.TEXTURE_2D, textureArray[6].textureWebGL)
                gl.uniform1i(gl.getUniformLocation(program, "albedoMap"), 6)
...
function spawnAstreoid()
{
    ...
    setColor(vec4(.9, .9, .9, 1))

    gl.activeTexture(gl.TEXTURE8)
    gl.bindTexture(gl.TEXTURE_2D, textureArray[8].textureWebGL)
    gl.uniform1i(gl.getUniformLocation(program, "albedoMap"), 8)

    if(TIME < 30)
    {
        gPush()
        {
            gTranslate(-19+TIME/2,0,0) ;
            s = .5
            gScale(2*s, s/2, s/2)
                gRotate(90, 0, 1, 0)
                drawCone()
...
function spawnExplosion()
{
    if ( TIME > 30)
    {
        ...
        gl.activeTexture(gl["TEXTURE"+(9+tile_index)])
        gl.bindTexture(gl.TEXTURE_2D, textureArray[9+tile_index].textureWebGL)
        gl.uniform1i(gl.getUniformLocation(program, "albedoMap"), (9+tile_index))
